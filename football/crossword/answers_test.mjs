...
import { onRequestGet as answersPage } from "../../functions/football/crossword/answers/[no].js";
import { onRequestGet as answersIndex } from "../../functions/football/crossword/answers/index.js";
...
/* The functions read the shared rule from source, not a private copy. */
{
  ...
  for (const f of ["functions/football/crossword/answers/[no].js", "functions/football/crossword/answers/index.js"]) {
...
console.log("\nA released board is a real page");
{
  ...
  t("and a canonical of its own", body.includes(`/football/crossword/answers/${released}"`));
  t("and is indexable", !/noindex/.test(body));
  t("and links to today's board — the whole point",
    /href="\/football\/crossword\/"/.test(body));
}
...
console.log("\nThe index");
{
  ...
  const links = (body.match(/\/football\/crossword\/answers\/\d+/g) || [])
  ...
  if (!links.length) {
    t("the empty state points at today's board", /href="\/football\/crossword\/"/.test(body));
  }
}
