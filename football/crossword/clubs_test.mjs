...
t("clubs link to their own page", idxHtml.includes('href="/football/crossword/club/arsenal/"'));
t("themes link under /theme/, not /club/", idxHtml.includes('href="/football/crossword/theme/grounds/"'));
/* Arsenal and Arsenal — Wenger Era are one club with two sets, not two clubs. */
t("a club and its sub-themes count as one entry",
  (idxHtml.match(/href="\/football\/crossword\/club\//g) || []).length === 2,
  (idxHtml.match(/href="\/football\/crossword\/club\//g) || []).length + " club links for 2 clubs");
t("it is indexable", !/noindex/.test(idxHtml) && !idx.headers.get("X-Robots-Tag"));
t("and links back to the sheet for requests", idxHtml.includes("/football/crossword/?themes=1"));
...
t("it lists both released boards",
  clubHtml.includes("/football/crossword/club/arsenal/1") && clubHtml.includes("/football/crossword/club/arsenal/2"));
...
t("and every number is its own target, not a label",
  rows.every((r) => r.nos.every((n) =>
    clubHtml.includes('class="no" href="/football/crossword/club/'))) &&
  clubHtml.includes('/football/crossword/club/arsenal-wenger-era/2"'));
t("each set's numbers point at that set, not at the club",
  clubHtml.includes('href="/football/crossword/club/arsenal-midfielders/1"'));
...
t("it carries a canonical url",
  clubHtml.includes('rel="canonical" href="https://www.thexigames.com/football/crossword/club/arsenal/"'));
...
t("naming the board, so the game opens it",
  door.headers.get("Location") === "https://www.thexigames.com/football/crossword/?t=arsenal-1",
...
t("to the club page, not to itself",
  setUrl.headers.get("Location") === "https://www.thexigames.com/football/crossword/club/arsenal/");
...
t("the bare tree sends you to the index",
  root.status === 301 && root.headers.get("Location").endsWith("/football/crossword/clubs/"));
...
t("pathOf agrees with the links the pages write",
  pathOf({ kind: "club", id: "arsenal" }) === "/football/crossword/club/arsenal/" &&
  pathOf({ kind: "topic", id: "grounds" }) === "/football/crossword/theme/grounds/");
