...
const page = await get("/football/scrambled/");
...
t("the sitemap lists it", (await map.text()).indexOf("/football/scrambled/</loc>") > -1);
...
/* The whole contract a linking bot depends on, checked against production:
   /football/scrambled/daily lands on a dated address, that address serves the game, and a
   ...
   in node, which has no Workers runtime to run one in. */
{
  ...
  const hop = await fetch(BASE + "/football/scrambled/daily", { redirect: "manual" });
  const link = hop.headers.get("link") || "";
  const key = (link.match(/\/football\/scrambled\/daily\/([^>]+)>/) || [])[1] || "";
  t("/football/scrambled/daily serves today, with no number in the address",
  ...
  const page = await fetch(BASE + "/football/scrambled/daily/" + key, { redirect: "manual" });
  ...
  t("with a base, so its relative assets still resolve",
    html.includes('<base href="/football/scrambled/">'));
  t("naming the board in its title and its canonical",
    /<title>[^<]+ \u00b7 /.test(html) && html.includes("/football/scrambled/daily/" + key + '"'));
  t("and offered to a crawler with a line of its own",
    !/noindex/.test(html) && /name="description" content="[^"]*\d{4}"?/.test(html));

  const future = await fetch(BASE + "/football/scrambled/daily/99999", { redirect: "manual" });
