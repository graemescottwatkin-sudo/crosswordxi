...
t("the hub links to it", hubHtml.indexOf('href="/vowels/"') > -1);
const map = await get("/sitemap.xml");
t("the sitemap lists it", (await map.text()).indexOf("/vowels/</loc>") > -1);
...
/* The whole contract a linking bot depends on, checked against production:
   /vowels/daily lands on a dated address, that address serves the game, and a
   ...
   in node, which has no Workers runtime to run one in. */
{
  ...
  const hop = await fetch(BASE + "/vowels/daily", { redirect: "manual" });
  const link = hop.headers.get("link") || "";
  const key = (link.match(/\/vowels\/daily\/([^>]+)>/) || [])[1] || "";
  t("/vowels/daily serves today, with no number in the address",
  ...
  const page = await fetch(BASE + "/vowels/daily/" + key, { redirect: "manual" });
