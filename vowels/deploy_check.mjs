...
const LAST_SHIPPED_ASSETS = "e942d83e513608d3";
...
function ownAssetHash() {
  ...
  for (const p of paths) {
    ...
    /* NORMALISED TO LF FIRST. The hash is meant to describe the bytes that
       ...
       agree. */
    h.update(fs.readFileSync(path.join(ROOT, "vowels", p), "utf8").replace(/\r\n/g, "\n"));
  }
...
/* ITS OWN ASSETS, ANCHORED ON THE QUOTE. Inherited from Scrambled's gate,
   ...
   ../scrambled/js/ deliberately — one file, one cache entry, the same rules
...
t("and the rules it borrows stay on Scrambled's tag",
  /\.\.\/scrambled\/js\/(?:config|scoring)\.js\?v=v[0-9a-z]+"/.test(html) &&
  !new RegExp("\.\./scrambled/js/(?:config|scoring)\.js\?v=" + tag + '"').test(html),
...
t("no forbidden files in the game folder", (() => {
  const bad = fs.readdirSync(path.join(ROOT, "vowels"))
...
t("no board source is served from the game folder",
  !has("vowels/xi") && !has("vowels/boards"),
...
t("the generated module lives under functions/, which Pages bundles",
  has("functions/_lib/sc-boards.js") && !has("vowels/js/sc-boards.js"));
...
t("it defines no .xic- rules of its own", (() => {
  const css = read("vowels/css/style.css");
  let out = "";
