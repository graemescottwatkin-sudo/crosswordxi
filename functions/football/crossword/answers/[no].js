/* GET /football/crossword/answers/:no
 *
...
function shell(title, desc, canonical, body, index) {
  return sitePage({
    ...
    noindex: !index, current: "/football/crossword/answers/", extraCss: OWN_CSS,
  });
}
...
export async function onRequestGet({ params, env }) {
  ...
  if (!answersAvailable(no, today)) {
    ...
    return html(shell(
      ...
      "https://www.thexigames.com/football/crossword/answers/",
...
<nav><a href="/football/crossword/answers/">All published answers</a></nav>`,
      false), false, 404);
  }
  ...
  if (!stored || !stored.puzzle) {
    return html(shell("Board not found \u00b7 Crossword XI",
      ...
      "https://www.thexigames.com/football/crossword/answers/",
      `<h1>No board #${no}</h1><nav><a href="/football/crossword/answers/">All published answers</a></nav>`,
      false), false, 404);
  }
...
<details><summary>Show answer</summary><p class="ans">${esc(e.row.answer)}</p></details></li>`;
  ...
  const canonical = `https://www.thexigames.com/football/crossword/answers/${no}`;
...
<a class="cta ghost" href="/football/crossword/daily/${no}">Play board #${no}</a>
<nav>
${answersAvailable(no - 1, today) ? `<a href="/football/crossword/answers/${no - 1}">Board #${no - 1}</a>` : ""}
${answersAvailable(no + 1, today) ? `<a href="/football/crossword/answers/${no + 1}">Board #${no + 1}</a>` : ""}
<a href="/football/crossword/answers/">All published answers</a>
</nav>`;
